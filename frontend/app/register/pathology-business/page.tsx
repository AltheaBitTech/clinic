...
import {
  FlaskConical, Phone, Mail, MapPin, Building2, User, Gift,
...
function PathologyBusinessRegisterForm() {
  const [form, setForm] = useState<FormData>(INITIAL);
  const [errors, setErrors] = useState<Partial<Record<keyof FormData, string>>>({});

  const submitMutation = useMutation({
    mutationFn: (data: FormData) => tenantRequestsApi.create({ ...data, type: 'PATHOLOGY' }),
    onError: (err: any) => {
      toast.error(
  ...
  const validate = (): boolean => {
    ...
    if (!form.name.trim()) errs.name = 'Lab business name is required';
    ...
    return (
      <PageShell>
        <StatusCard
          ...
          description="Our team will review your pathology lab registration shortly. You'll receive your login details by email once it's approved."
        />
      </PageShell>
    );
  }
  ...
  return (
    <PageShell>
      <div className="text-center mb-6">
        <h1 className="text-3xl font-extrabold text-white tracking-tight">Register your pathology lab</h1>
        <p className="text-slate-400 mt-2 text-sm font-light max-w-md mx-auto leading-relaxed">
          Sign up your diagnostic lab as an independent account on Arogyix — no hospital affiliation required. You can link with hospitals later.
        </p>
      </div>
      ...
      <div className="bg-white/[0.03] border border-white/10 rounded-3xl p-6 sm:p-8 backdrop-blur-md shadow-2xl relative overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="space-y-6" id="pathology-business-register-form">
          <FormSection icon={FlaskConical} title="Business Information" description="Core details about your lab">
            <FormField id="pathology-business-name" label="Lab Business Name" required error={errors.name}>
              <input
                id="pathology-business-name"
                value={form.name}
          ...
          <FormSection icon={Phone} title="Contact Details" description="How we can reach you">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField id="pathology-business-phone" label="Phone Number" required error={errors.phone}>
                <div className="relative">
                  <Phone className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
                  <input
                    id="pathology-business-phone"
                    value={form.phone}
          ...
          <FormSection icon={MapPin} title="Address & Location" description="Physical location of the lab">
            <FormField id="pathology-business-address" label="Street Address" required error={errors.address}>
              <div className="relative">
                <MapPin className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
                <input
                  id="pathology-business-address"
                  value={form.address}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField id="pathology-business-city" label="City">
                <div className="relative">
                  <Building2 className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
                  <input
                    id="pathology-business-city"
                    value={form.city}
              ...
              <FormField id="pathology-business-state" label="State">
                <input
                  id="pathology-business-state"
                  value={form.state}
          ...
          <FormSection icon={User} title="Owner Details" description="Who we'll create the login for">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField id="pathology-business-first-name" label="First Name" required error={errors.firstName}>
                <input
                  id="pathology-business-first-name"
              ...
              <FormField id="pathology-business-last-name" label="Last Name" required error={errors.lastName}>
                <input
                  id="pathology-business-last-name"
                  value={form.lastName}
          ...
          <FormSection icon={Gift} title="Referral (optional)" description="Were you referred by an Arogyix partner?">
            <FormField id="pathology-business-referral-code" label="Referral Code" hint="Leave blank if you don't have one">
              <input
                id="pathology-business-referral-code"
          ...
          <button
            id="submit-pathology-business-registration-btn"
            type="submit"
...
export default function PathologyBusinessRegisterPage() {
  return <PathologyBusinessRegisterForm />;
}
