...
import {
  FlaskConical, Phone, Mail, MapPin, Clock, Truck, FileText, ShieldCheck,
...
function PathologyLabRegisterForm() {
  ...
  const inviteQuery = useQuery({
    queryKey: ['pathology-lab-invite', token],
    queryFn: () => pathologyLabsApi.getInvite(token as string).then((r) => r.data),
    enabled: !!token,
    retry: false,
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const optionalFields: (keyof FormData)[] = [
      'ownerName', 'licenseNumber', 'accreditationNo', 'city', 'state', 'pincode',
      'openingHours', 'closingHours', 'notes',
    ];
    ...
    return (
      <PageShell>
        <StatusCard
          ...
          description="This link is missing its invite token. Ask the hospital that invited you for a fresh registration link."
        />
      </PageShell>
    );
  }
    ...
    return (
      <PageShell>
        <StatusCard
          ...
          description={`${form.name} has been registered as an independent lab and linked with ${inviteQuery.data?.tenantName || 'the hospital'}. Sign in with ${form.email} to manage your listing.`}
        />
      </PageShell>
    );
  }
  ...
  return (
    <PageShell>
      <div className="text-center mb-6">
        <h1 className="text-2xl font-bold text-slate-800">Register your pathology lab</h1>
        <p className="text-slate-500 text-sm mt-1.5">
          You&apos;ve been invited by{' '}
          <span className="font-semibold text-slate-700">
            {inviteQuery.data?.tenantName}
          </span>
          {' '}to join Arogyix as an independent lab. Fill in your details below — you can
          link with other hospitals later too.
        </p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-6" id="pathology-lab-self-register-form">
        <FormSection icon={FlaskConical} title="Basic Information" description="Core details about the lab">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField id="lab-name" label="Lab Name" required error={errors.name}>
              <input
                id="lab-name"
                value={form.name}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <FormField id="accreditation-no" label="Accreditation No. (e.g. NABL)" error={errors.accreditationNo}>
              <div className="relative">
        ...
        <FormSection icon={Phone} title="Contact Details" description="How hospitals and patients can reach you">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField id="lab-phone" label="Phone Number" required error={errors.phone}>
              <div className="relative">
            ...
            <FormField id="lab-email" label="Email Address" required error={errors.email}>
              <div className="relative">
        ...
        <FormSection icon={MapPin} title="Address & Location" description="Physical location of the lab">
          <FormField id="lab-address" label="Street Address" required error={errors.address}>
            <div className="relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <FormField id="lab-city" label="City">
              <div className="relative">
            ...
            <FormField id="lab-state" label="State">
              <input
                id="lab-state"
                value={form.state}
            ...
            <FormField id="lab-pincode" label="Pincode">
              <input
                id="lab-pincode"
                value={form.pincode}
          ...
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Enable if you collect samples from patients&apos; homes
              </p>
            </div>
            <div
        ...
        <FormSection icon={FileText} title="Additional Notes" description="Anything else hospitals should know">
          <textarea
            ...
            placeholder="e.g. NABL accredited, reports in 24h…"
            rows={3}
            className="input resize-none"
          />
        </FormSection>

        <FormSection icon={KeyRound} title="Create Your Login" description="Used to sign in and manage your lab listing">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <FormField id="lab-first-name" label="Your First Name" required error={errors.firstName}>
              <div className="relative">
                <User className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                <input
                  id="lab-first-name"
            ...
            <FormField id="lab-last-name" label="Your Last Name" required error={errors.lastName}>
              <div className="relative">
                <User className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                <input
                  id="lab-last-name"
          ...
          <FormField id="lab-password" label="Password" required error={errors.password}>
            <div className="relative">
        ...
        <button
          id="submit-pathology-lab-registration-btn"
          type="submit"
...
export default function PathologyLabRegisterPage() {
  return (
    <Suspense
      fallback={
