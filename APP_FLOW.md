# CareSync — App Flow, Roles, and Demo Script

This document details the multi-tenant SaaS architecture of **CareSync**, the granular permissions of all five user roles, the core business flows, and a step-by-step script to test or demo the end-to-end application lifecycle.

---

## 🏗️ System & Tenant Architecture

CareSync is built on a **multi-tenant architecture** where every hospital or clinic is an isolated tenant:
...
CareSync defines 5 distinct user roles. Below is the mapping of each role's dashboard routes, responsibilities, and permissions:
...
- **Super Admin**: `superadmin@caresync.health` / `Password123!`
- **Hospital Admin**: `admin@caresync.health` / `Password123!`
- **Doctor**: `doctor@caresync.health` / `Password123!`
- **Receptionist**: `receptionist@caresync.health` / `Password123!`
- **Patient**: `patient@caresync.health` / `Password123!`
...
1. **Submit Signup Request**:
   ...
   - Enter details for a new hospital (e.g., `City Hospital`, admin email `cityadmin@caresync.health`, first/last name, phone).
   - Submit the form.
2. **Approve Request**:
   - Navigate to `http://localhost:3000/login` and log in as the **Super Admin** (`superadmin@caresync.health` / `Password123!`).
   - Go to `/dashboard/super-admin` to view the **Pending Requests** table.
   - Find the entry for `City Hospital` and click **Approve**.
   - *Verification*: The request status changes to `Approved`. A new tenant record for `City Hospital` is provisioned, and the admin account `cityadmin@caresync.health` is registered.
...
1. **Log in as Hospital Admin**:
   - Log out of Super Admin and log back in using the default Hospital Admin: `admin@caresync.health` / `Password123!`.
2. **Add a Department**:
...
3. **Invite / Register Staff**:
   ...
   - Click **Add Staff Member** and select the **Doctor** role. Enter credentials (e.g., email `cardio_doc@caresync.health`, assign to `Cardiology`).
   - Add another staff member with the **Receptionist** role (e.g., email `frontdesk@caresync.health`).
...
1. **Log in as Receptionist**:
   - Log in with `receptionist@caresync.health` / `Password123!`.
...
3. **Book an Appointment**:
   ...
   - Choose the doctor (e.g., `doctor@caresync.health` or the newly created doctor).
...
1. **Log in as Doctor**:
   - Log in with `doctor@caresync.health` / `Password123!`.
2. **Examine Patient**:
...
1. **Log in as Receptionist**:
   - Log back in with `receptionist@caresync.health` / `Password123!`.
2. **Collect Payment**:
...
1. **Log in as Patient**:
   - Log in with `patient@caresync.health` / `Password123!`.
...
3. **Use Patient-Doctor Chat**:
   ...
   - Log back in as the **Doctor** (`doctor@caresync.health`) and verify the message appears in real-time via Socket.io. Respond back to complete the loop.
